...
def estimate_optimal_cutoff(y_true, y_pred_proba):
    ...
    best_acc, opt_thresh = 0.0, 0.5
    for cutoff_value in np.linspace(0.4, 0.6, 101):
        acc = accuracy_score(y_true, (y_pred_proba >= cutoff_value).astype(int))
        if acc > best_acc:
            best_acc, opt_thresh = acc, cutoff_value
    return opt_thresh, best_acc


def analyze_meta_model_weights(best_tier1_tag, winning_model, X_tier1_test: pd.DataFrame):
    ...
    if best_tier1_tag == 'XGBoost_Meta':
        ...
        importance_df = pd.DataFrame({
            ...
            by='Peso Lineare (Coef)',
            key=np.abs,
            ascending=False
        )

        print("Pesi Lineari delle Predizioni (Features) Nello Stacking:\n")
        print(importance_df.to_string(index=False, float_format='%.4f'))
