# --- Model definition and stacking for 12-feature model ---
import numpy as np
import pandas as pd
...
def train_stacking_pipeline(train_df: pd.DataFrame, test_df: pd.DataFrame):
    ...
    # Define a compact subset of features for the LR_Lite model
    SUBSET_FEATURES = [
    ...
    # Add XGB base model if available
    if HAS_XGB:
    ...
    # Evaluate meta models on OOF data and optimize threshold
    for meta_tag, meta_model in TIER1_MODELS.items():
        ...
        accs_per_t = [
            accuracy_score(y_target, (oof_meta_proba >= t).astype(int))
            for t in threshold_grid
        ]
    ...
    # Return everything needed for final refit and submission
    return (
        TIER0_MODELS,
        META_RESULTS,
        best_meta_tag,
