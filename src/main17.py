...
SUBSET_FEATURES = ['hp_avg_delta', 'survivor_count_delta', 'team_status_net_delta', 'total_weighted_power_delta',
                   'switch_activity_log_ratio', 'stat_dom_diff_metric', 'lead_hp_frac_diff', 'lead_status_value_diff',
...
ALL_FEATURES = [col for col in train_features_df.columns if col not in ['battle_id', 'player_won']]
...
TIER0_MODELS = {
    'LR_Full': (X_full, X_test_full, define_linear_grid_search(X_full)),
    'LR_Lite': (X_lite, X_test_lite, define_linear_grid_search(X_lite)),
    'RF_Model': (X_full, X_test_full, define_non_linear_random_search(RFC(random_state=42), RF_SEARCH_SPACE)),
    'HGBT_Model': (X_full, X_test_full, define_non_linear_random_search(HGBC(random_state=42), HGBT_SEARCH_SPACE)),
    'XGB_Model': (X_full, X_test_full, define_non_linear_random_search(XGBC(objective='binary:logistic', use_label_encoder=False, eval_metric='logloss', random_state=42), XGB_SEARCH_SPACE)),
}
...
skf_splitter = StratifiedKFold(n_splits=NUM_FOLDS, shuffle=True, random_state=42)
test_set_oof_preds = {}
...
for model_tag, (X_source, X_test_source, model_pipe) in TIER0_MODELS.items():
    ...
    for fold_id, (train_indices, val_indices) in enumerate(skf_splitter.split(X_source, y_target)):
        X_train_fold, X_val_fold = X_source.iloc[train_indices], X_source.iloc[val_indices]
        ...
        test_preds_sum += model_pipe.predict_proba(X_test_source)[:, 1] / NUM_FOLDS
...
# ==========================================================
TIER1_MODELS_DEF['Logistic_Meta'] = LogisticRegression(C=0.5, solver='liblinear', random_state=42)
X_tier1_test = pd.DataFrame(test_set_oof_preds)

best_tier1_tag, best_tier1_acc, final_optimal_cutoff = "", 0, 0
...
for tier1_tag, tier1_model in TIER1_MODELS_DEF.items():
    ...
    opt_cutoff, opt_acc = estimate_optimal_cutoff(y_target, tier1_oof_probs)

    if opt_acc > best_tier1_acc:
        best_tier1_acc = opt_acc
        best_tier1_tag = tier1_tag
        final_optimal_cutoff = opt_cutoff
        final_tier1_test_probs = tier1_model.predict_proba(X_tier1_test)[:, 1]

# Analisi dei pesi
winning_model = TIER1_MODELS_DEF[best_tier1_tag]
analyze_meta_model_weights(best_tier1_tag, winning_model, X_tier1_test)

# Risultato Finale e Submission
final_test_predictions = (final_tier1_test_probs >= final_optimal_cutoff).astype(int)
